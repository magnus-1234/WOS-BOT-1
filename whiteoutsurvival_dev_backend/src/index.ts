...
const getCollections = async () => {
  ...
  const gameMapPlans = db.collection<GameMapPlanDocument>('game_map_plans');
  ...
  indexesReady ??= Promise.all([
    ...
    gameMapPlans.createIndex({ id: 1 }, { unique: true }),
    foundryPlans.createIndex({ creatorUserId: 1, createdAt: -1 }),
    gameMapPlans.createIndex({ creatorUserId: 1, createdAt: -1 }),
  ]).then(() => undefined);
